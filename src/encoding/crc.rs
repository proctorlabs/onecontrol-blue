#[allow(dead_code)]
pub struct CRC8(u8);

#[allow(dead_code)]
impl CRC8 {
    // Dallas Maxim CRC8, 0x55 initial value
    const CRC_TABLE: &'static [u8] = &[
        ,
    ];
    pub fn new() -> Self {
        return CRC8(0x55);
    }

    pub fn calc(data: &[u8]) -> u8 {
        let mut crc = CRC8::new();
        for b in data {
            crc.next(*b);
        }
        crc.cur()
    }

    pub fn next(&mut self, b: u8) {
        self.0 = Self::CRC_TABLE[((b ^ self.cur()) & 0xFF) as usize];
    }

    pub fn cur(&self) -> u8 {
        self.0
    }
}

#[cfg(test)]
mod tests {
    const INPUTS: &[&[u8]] = &[
        &[0x00, 0x06u8, 0x03, 0x01, 0x10, 0xFF, 0xFF],
        &[0x21, 0x06u8, 0x03, 0x31, 0x10, 0x33, 0x44],
        &[0x55, 0x02u8, 0x03, 0x66, 0x10, 0x11, 0x01],
    ];
    const INPUTS_CRC: &[u8] = &[0x78, 0x09, 0x85];

    #[test]
    fn sanity_test() {
        for (i, (data, expected_crc)) in INPUTS.iter().zip(INPUTS_CRC).enumerate() {
            let calculated_crc = super::CRC8::calc(data);
            assert_eq!(*expected_crc, calculated_crc);
            println!("Check {i} passed!")
        }
    }
}
